...
- 追踪节奏为每月一次（默认每月 1 日，`PI_MONTHLY_RUN_DAY/HOUR/MINUTE` 可调）；发现窗口是以扫描时刻为终点的近 30 天滚动区间，候选页会明确展示该日期区间，只有在区间内以日精度发表的才算“本月新论文”。当月新论文不足五篇时按顺序回补：先用本次扫描首次发现但非本月发表的主题论文（“本月补发现”，按发表时间降序），再用往期未读回补，最后才用经典回顾补位；每类都有明确标签，不会把老论文伪装成本月新发现。
- 候选审阅页有独立的「近年高引 · 未读经典」栏目：从注册刊物中确定性拉取近 4 年、与项目主题相关的高引论文（OpenAlex 题录，不调模型、不占五篇候选、不预下载全文），已读、已收藏和标过“不感兴趣”的不会再出现；覆盖范围限于有可靠 OpenAlex 来源标识的刊物并如实标注。每篇可一键加入本月推荐（之后照常走全文准备）或标记不感兴趣（持久排除，也作用于往期未读回补）。
...
- 候选与推荐论文展示忠实的中文标题翻译（保留必要英文术语与原标题）与发表时间；本月新论文不足时，先回补往期未读且未处理的优质推荐（按稳定论文身份去重、标注为回补），再用经典补位；候选审阅页可回看往期推荐及当时的处理状态。主题检索里未被推荐的论文标题、以及联网发现的英文标题与摘要，会统一用 Codex 5.3 Spark 做一道尽力而为的中文翻译（失败则保留英文原文，不阻断结果）。

...
- 主题检索是每周追踪之外的独立检索会话，支持新建、切换、删除多个会话并保留历史（删除前会弹出确认，已加入推荐的论文不受影响）；左栏主题检索入口可一键展开/收起会话列表。提问后实时展现分阶段进度（规划关键词 → 检索刊物与联网 → 阅读命中整理推荐）与面向用户的思考摘要；确定性适配器只在 11 个注册刊物内检索，并行的联网发现只作参考展示、不能加入本周推荐或归档；采纳的注册刊物论文进入当周候选管线。当模型规划失败时会先重试一次，再从问题中提取英文术语兜底检索（不把整段中文发给学术索引）。联网发现与正常工作的联网搜索共用同一份豆包月度免费额度台账，跨进程、崩溃后都不会超额，超限或失败时自动回退 Tavily；联网发现失败只降级为仅展示刊物结果，不会让整次检索失败。

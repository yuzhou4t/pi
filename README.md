...
- 候选审阅页有独立的「近年高引 · 未读经典」栏目：从注册刊物中确定性拉取近 4 年、与项目主题相关的高引论文（OpenAlex 题录，不调模型、不占五篇候选、不预下载全文），已读、已收藏和标过“不感兴趣”的不会再出现；覆盖范围限于有可靠 OpenAlex 来源标识的刊物并如实标注。标题与摘要会统一用 Codex 5.3 Spark（`gpt-5.3-codex-spark`，`PI_JOURNAL_TRANSLATION_MODEL` 可覆盖）翻成中文，让这里和每月追踪一样能看懂大概内容（翻译在整理栏目时进行，失败保留英文；已落盘的旧数据可在页面上点「翻译」一键补上中文）。每篇可一键加入本月推荐（之后照常走全文准备）或标记不感兴趣（持久排除，也作用于往期未读回补）。
- 「近年经典」和「往期回看」是左栏独立入口。往期回看列出历次推荐及其中文说明与处理状态；未处理的往期论文可一键加入本月推荐（按稳定论文身份去重、标注为“往期未读回补”，随后照常准备全文）。近年经典与往期论文只能加入当月已进入审阅的 Run。
- 本月推荐支持刷新：候选审阅页有「刷新本月推荐」按钮，月内每周也会自动刷新一次。刷新是一次独立命名空间的增量扫描，只把窗口内新发表、未重复、未被排除的论文追加进本月候选，不覆盖首次扫描的证据、不移除已选择或已处理的论文。
- 主题检索是每月追踪之外的独立检索会话，支持新建、切换、删除多个会话并保留历史（删除前会弹出确认，已加入推荐的论文不受影响）；左栏主题检索入口可一键展开/收起会话列表。提问后实时展现分阶段进度（规划关键词 → 检索刊物与联网 → 阅读命中整理推荐）与面向用户的思考摘要；模型会一次规划 1–4 组互补检索式（同义词/子方向/方法名等不同角度），分别在 11 个注册刊物内检索、在原始记录层合并去重后再排序，提高相关与启发性内容的召回；并行的联网发现只作参考展示、不能加入本月推荐或归档；采纳的注册刊物论文进入当周候选管线。当模型规划失败时会先重试一次，再从问题中提取英文术语兜底检索（不把整段中文发给学术索引）。联网发现与正常工作的联网搜索共用同一份豆包月度免费额度台账，跨进程、崩溃后都不会超额，超限或失败时自动回退 Tavily；联网发现失败只降级为仅展示刊物结果，不会让整次检索失败。
- 候选与推荐论文展示忠实的中文标题翻译（保留必要英文术语与原标题）与发表时间；本月新论文不足时，先回补往期未读且未处理的优质推荐（按稳定论文身份去重、标注为回补），再用经典补位；候选审阅页可回看往期推荐及当时的处理状态。主题检索里未被推荐的论文标题、以及联网发现的英文标题与摘要，会统一用 Codex 5.3 Spark 做一道中文翻译（失败则保留英文原文，不阻断结果）。

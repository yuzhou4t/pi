...
- 主题检索是每周追踪之外的独立检索会话，支持新建、切换、删除多个会话并保留历史；旧的单一检索记录会自动迁移。提问后由模型规划关键词，确定性适配器只在 11 个注册刊物内检索，并行的联网发现只作参考展示、不能加入本周推荐或归档；采纳的注册刊物论文进入当周候选管线。联网发现与正常工作的联网搜索共用同一份豆包月度免费额度台账，跨进程、崩溃后都不会超额，超限或失败时自动回退 Tavily；联网发现失败只降级为仅展示刊物结果，不会让整次检索失败。
- 精读以“整篇导读 + 按需提问”为主，不再显示或强制 `N/4` 阶段笼子。十轮引导记录稳定 `round_id`；短答、旧内部字段和待核验记录保留审计但不进入正常上下文。

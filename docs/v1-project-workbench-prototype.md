...
Pi Agent 在论文闭环之外增加一种同级的项目工作会话。用户可以绑定本地项目、显式提交普通 Agent 任务、查看公开计划与执行活动，并在右侧审阅文件、更改、预览和运行结果。

V1 不再预置固定任务；用户绑定任意受支持的本地项目后显式提交任务，走以下路径：
...
- 左上先选择 `正常工作` 或 `论文精读`，其下采用接近 Codex 的项目树：展示当前类型的多个项目及项目内会话。
...
- V1 的删除是有确认的永久删除，不伪装成没有恢复入口的“归档”。确认框必须列明会删除对话、计划、运行记录和未应用修改草稿，且不会删除项目文件夹或回滚已经确认的修改；若存在待确认 ChangeSet，还要显示未应用文件数量。
...
- `Conversation` 保存状态、Pi provider/model、消息、计划、待确认 ChangeSet 与验证记录。
